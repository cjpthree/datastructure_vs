 ...
 static Status CreateGraph(ALGraph &G)
 ...
 static Status PutVex(ALGraph &G,VertexType v,VertexType value)
 { // 初始条件: 图G存在,v是G中某个顶点
   // 操作结果: 对v赋新值value
   int i;
   i=LocateVex(G,v);
 ...
 static Status DeleteVex(ALGraph &G,VertexType v)
 { // 初始条件: 图G存在,v是G中某个顶点
   // 操作结果: 删除G中顶点v及其相关的弧
   int i,j;
   ArcNode *p,*q;
 ...
 static Status InsertArc(ALGraph &G,VertexType v,VertexType w)
 ...
 static Status DeleteArc(ALGraph &G,VertexType v,VertexType w)
 { // 初始条件: 图G存在,v和w是G中两个顶点
 ...
 static void(*VisitFunc)(char* v); // 函数变量(全局量)
 ...
 static void DFSTraverse(ALGraph G,void(*Visit)(char*))
 { // 对图G作深度优先遍历。算法7.4
   int v;
 ...
 #include"c3-2.h"
 #include"bo3-2.cpp"
 static void BFSTraverse(ALGraph G,void(*Visit)(char*))
