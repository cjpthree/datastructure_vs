 ...
 static int LocateVex(AMLGraph G,VertexType u)
 ...
 static Status CreateGraph(AMLGraph &G)
 { // 采用邻接多重表存储结构,构造无向图G
   int i,j,k,l,IncInfo;
   char s[MAX_INFO];
   VertexType va,vb;
 ...
 static VertexType& GetVex(AMLGraph G,int v)
 ...
 static Status PutVex(AMLGraph &G,VertexType v,VertexType value)
 ...
 static int FirstAdjVex(AMLGraph G,VertexType v)
 { // 初始条件: 无向图G存在,v是G中某个顶点
 ...
 static int NextAdjVex(AMLGraph G,VertexType v,VertexType w)
 ...
 static Status InsertVex(AMLGraph &G,VertexType v)
 ...
 static Status DeleteArc(AMLGraph &G,VertexType v,VertexType w)
 ...
 static Status DeleteVex(AMLGraph &G,VertexType v)
 { // 初始条件: 无向图G存在,v是G中某个顶点
   // 操作结果: 删除G中顶点v及其相关的边
   int i,j;
   VertexType w;
 ...
 static void DestroyGraph(AMLGraph &G)
 {
   int i;
 ...
 static Status InsertArc(AMLGraph &G,VertexType v,VertexType w)
 { // 初始条件: 无向图G存在,v和W是G中两个顶点
   // 操作结果: 在G中增添弧<v,w>
   int i,j,l,IncInfo;
   char s[MAX_INFO];
 ...
 static Boolean visite[MAX_VERTEX_NUM]; // 访问标志数组(全局量)
 static Status(*VisitFunc)(VertexType v);
 ...
 static void DFSTraverse(AMLGraph G,Status(*visit)(VertexType))
 ...
 static void BFSTraverse(AMLGraph G,Status(*Visit)(VertexType))
 ...
 static void MarkUnvizited(AMLGraph G)
 { // 置边的访问标记为未被访问
   int i;
   EBox *p;
   for(i=0;i<G.vexnum;i++)
 ...
 static void Display(AMLGraph G)
 { // 输出无向图的邻接多重表G
   int i;
   EBox *p;
