 ...
 static int LocateVex(OLGraph G,VertexType u)
 ...
 static VertexType& GetVex(OLGraph G,int v)
 ...
 static Status PutVex(OLGraph &G,VertexType v,VertexType value)
 ...
 static int FirstAdjVex(OLGraph G,VertexType v)
 { // 初始条件: 有向图G存在,v是G中某个顶点
 ...
 static int NextAdjVex(OLGraph G,VertexType v,VertexType w)
 ...
 static void InsertVex(OLGraph &G,VertexType v)
 ...
 static Status DeleteVex(OLGraph &G,VertexType v)
 { // 初始条件: 有向图G存在,v是G中某个顶点
   // 操作结果: 删除G中顶点v及其相关的弧
   int j,k;
   ArcBox *p,*q;
 ...
 static Status InsertArc(OLGraph &G,VertexType v,VertexType w)
 ...
 static Status DeleteArc(OLGraph &G,VertexType v,VertexType w)
 { // 初始条件: 有向图G存在,v和w是G中两个顶点
   // 操作结果: 在G中删除弧<v,w>
   int i,j;
   ArcBox *p1,*p2;
 ...
 static Boolean visited[MAX_VERTEX_NUM]; // 访问标志数组
 static Status(*VisitFunc)(VertexType); // 函数变量
 static void DFS(OLGraph G,int i) // DFSTraverse()调用
 {
   ArcBox *p;
 ...
 static void DFSTraverse(OLGraph G,Status(*Visit)(VertexType))
 ...
 #include"c3-3.h"
 #include"bo3-3.cpp"
 static void BFSTraverse(OLGraph G,Status(*Visit)(VertexType))
