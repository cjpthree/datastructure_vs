 ...
 // 广义表的书写形式串为HString类型
 static Status InitGList(GList &L)
 { // 创建空的广义表L
   L=NULL;
   return OK;
 }

 static Status sever(HString &str,HString &hstr) // 同bo5-52.cpp
 ...
 static Status CreateGList(GList &L,HString S)
 ...
 static void DestroyGList(GList &L)
 ...
 static Status CopyGList(GList &T,GList L)
 ...
 static int GListLength(GList L)
 ...
 static Status GListEmpty(GList L)
 ...
 static Status InsertFirst_GL(GList &L,GList e)
 { // 初始条件: 广义表存在
 ...
 static Status DeleteFirst_GL(GList &L,GList &e)
 ...
 static void Traverse_GL(GList L,void(*v)(AtomType))
 { // 利用递归算法遍历广义表L
   GList hp;
   if(L) // L不空
   {
