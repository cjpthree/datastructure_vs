 ...
 static int LocateVex(MGraph G,VertexType u)
 ...
 static Status CreateFAG(MGraph &G)
 ...
 static Status CreateAG(MGraph &G)
 ...
 static Status CreateAN(MGraph &G)
 ...
 static Status CreateGraph(MGraph &G)
 ...
 static void DestroyGraph(MGraph &G)
 { // 初始条件: 图G存在。操作结果: 销毁图G
   int i,j;
   if(G.kind<2) // 有向
 ...
 static VertexType& GetVex(MGraph G,int v)
 ...
 static Status PutVex(MGraph &G,VertexType v,VertexType value)
 ...
 static int FirstAdjVex(MGraph G,VertexType v)
 { // 初始条件: 图G存在,v是G中某个顶点
 ...
 static int NextAdjVex(MGraph G,VertexType v,VertexType w)
 ...
 static void InsertVex(MGraph &G,VertexType v)
 ...
 static Status DeleteVex(MGraph &G,VertexType v)
 ...
 static Status InsertArc(MGraph &G,VertexType v,VertexType w)
 ...
 static Status DeleteArc(MGraph &G,VertexType v,VertexType w)
 { // 初始条件: 图G存在,v和w是G中两个顶点
 ...
 static Boolean visited[MAX_VERTEX_NUM]; // 访问标志数组(全局量)
 static Status(*VisitFunc)(VertexType); // 函数变量
 ...
 static void DFSTraverse(MGraph G,Status(*Visit)(VertexType))
 ...
 static void BFSTraverse(MGraph G,Status(*Visit)(VertexType))
