 ...
 static Status InitTree(PTree &T)
 { // 操作结果: 构造空树T
   T.n=0;
   return OK;
 }
 ...
 static Status CreateTree(PTree &T)
 { // 操作结果: 构造树T
   LinkQueue q;
   QElemType p,qq;
   int i=1,j,l;
 ...
 static Status TreeEmpty(PTree T)
 ...
 static TElemType Value(PTree T,int i)
 ...
 static Status Assign(PTree &T,TElemType cur_e,TElemType value)
 ...
 static TElemType Parent(PTree T,TElemType cur_e)
 ...
 static TElemType LeftChild(PTree T,TElemType cur_e)
 { // 初始条件: 树T存在,cur_e是T中某个结点
 ...
 static TElemType RightSibling(PTree T,TElemType cur_e)
 { // 初始条件: 树T存在,cur_e是T中某个结点
 ...
 static Status InsertChild(PTree &T,TElemType p,int i,PTree c)
 ...
 static Status deleted[MAX_TREE_SIZE+1]; // 删除标志数组(全局量)
 static void DeleteChild(PTree &T,TElemType p,int i)
 ...
 static void TraverseTree(PTree T,void(*Visit)(TElemType))
