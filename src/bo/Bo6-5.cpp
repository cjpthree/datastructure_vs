 ...
 static Status InitTree(CSTree &T)
 { // 操作结果: 构造空树T
   T=NULL;
   return OK;
 }
 ...
 static Status CreateTree(CSTree &T)
 { // 构造树T
 ...
 static Status TreeEmpty(CSTree T)
 ...
 static TElemType Value(CSTree p)
 { // 返回p所指结点的值
   return p->data;
 }

 static TElemType Root(CSTree T)
 ...
 static CSTree Point(CSTree T,TElemType s)
 ...
 static Status Assign(CSTree &T,TElemType cur_e,TElemType value)
 ...
 static TElemType LeftChild(CSTree T,TElemType cur_e)
 { // 初始条件: 树T存在,cur_e是T中某个结点
 ...
 static TElemType RightSibling(CSTree T,TElemType cur_e)
 { // 初始条件: 树T存在,cur_e是T中某个结点
 ...
 static Status InsertChild(CSTree &T,CSTree p,int i,CSTree c)
 ...
 static Status DeleteChild(CSTree &T,CSTree p,int i)
 ...
 static void PreOrderTraverse(CSTree T,void(*Visit)(TElemType))
 { // 先根遍历孩子－兄弟二叉链表结构的树T
   if(T)
   {
 ...
 static void PostOrderTraverse(CSTree T,void(*Visit)(TElemType))
 { // 后根遍历孩子－兄弟二叉链表结构的树T
   CSTree p;
   if(T)
   {
